...
const USAGE =
  "Usage: /codex-import <codex session id | rollout.jsonl path> [--session-id <id>] [--cwd <dir>] [--max-turns <n>] [--title <text>]";
...
/** Parse raw command input into { target, sessionId, cwd, maxTurns, title }. */
export function parseArgs(rawInput) {
  const tokens = tokenize(rawInput);
  const out = { target: null, sessionId: null, cwd: null, maxTurns: null, title: null };
...
export function apply(ctx) {
  ctx.effect(function* () {
    yield ctx.commands.register({
      name: "codex-import",
      description:
        "Import a codex CLI conversation as a new DSH session (/codex-import <session-id|rollout.jsonl> [--session-id <id>] [--cwd <dir>] [--max-turns <n>] [--title <text>])",
      input: {
        hint: "<codex session id | rollout.jsonl> [--session-id <id>] [--cwd <dir>] [--max-turns <n>] [--title <text>]",
      },
