...
const USAGE =
  "Usage: /codex-import <codex session id | rollout.jsonl path> [--session-id <id>] [--cwd <dir>]";
...
/** Parse raw command input into { target, sessionId, cwd }. */
export function parseArgs(rawInput) {
  const tokens = tokenize(rawInput);
  const out = { target: null, sessionId: null, cwd: null };
 ...
 */
export async function importRollout({ persistence, rolloutPath, cwd, sessionId }) {
  ...
  const { header, events } = buildSession(turns, { sessionId: id, cwd: targetCwd, createdAt });
  ...
  return { sessionId: id, events: events.length, turns: turns.length, title };
}
...
export function apply(ctx) {
  ctx.effect(function* () {
    yield ctx.commands.register({
      name: "codex-import",
      description:
        "Import a codex CLI conversation as a new DSH session (/codex-import <session-id|rollout.jsonl> [--session-id <id>] [--cwd <dir>])",
      input: { hint: "<codex session id | rollout.jsonl> [--session-id <id>] [--cwd <dir>]" },
