...
class SessionBuilder {
  ...
  assistantMessage(text, time, turn, step) {
    this.ev(
      "assistant/message",
      {
        turn,
 ...
 *
 * One codex turn maps to one DSH turn with a single step: every user message,
 * assistant message, tool call, and tool result of the codex turn is emitted
 * in timestamp order inside that step, then the turn is closed. The produced
 * sequence satisfies the DSH session invariants (turn/step nesting, seq
 * contiguity, tool/result preceded by its tool/call).
 *
    ...
    for (const item of items) {
      if (item.group === "message") {
        if (item.kind === "user") {
          ...
          builder.assistantMessage(item.text, item.ts, number, step);
        }
        continue;
      }
