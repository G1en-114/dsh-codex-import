 ...
 * `user/message`, `assistant/message`, `tool/result` — and every `tool/result`
 * must be preceded by an assistant message whose content declares that call id
 * as a `tool-call` block. Each codex tool call is therefore attached as a
 * `tool-call` block to the nearest preceding assistant message (a synthetic
 * assistant message carrying only the tool-call block is emitted when the call
 * has no commentary before it). The standalone `tool/call` events remain for
 * the invariant checker and the UI trajectory.
 *
